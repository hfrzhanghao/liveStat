...
function relayView01_zb(datas) {
	...
	html += "<table class=\"detailTable\">" +
					"<tr align=\"center\">" +
						"<td>ID</td>" +
						"<td colspan=\"15\">基本信息（实时），此通话务中用到的Relay现在的状态</td>" +
				    			...
				    			if(relayID!=undefined&&relayID!=null&&relayID!==""){
					    			if(relayID==key){
					    				if(value!=undefined&&value!=null&&value!==""){
								    		...
								    		//探测数　
								    		max_detect_num = value.max_detect_num;
								    		
								    		//连接数
								    		max_connet_num = value.max_connet_num;
								    		
								    		//会话数　
								    		max_session_num = value.max_session_num;
								    		
								    		//运营商　
								    		isp = value.isp;
								    		
								    		//运营商索引　
								    		isp_index = value.isp_index;
								    		
								    		//预留ip 
								    		iperf_ip = value.iperf_ip;
								    		
								    		//预留端口 
								    		iperf_port = value.iperf_port;
...
/**
 * “Relay信息”标签页第一个子标签“Relay状态”，被叫端数据
 * @param {}
 * data
 */
function relayView01_bz(datas) {
	var html = "";
	html += "<font style=\"color:red;font-weight:bold;\">★红色加粗</font>代表该relay超时未心跳&nbsp;&nbsp;<font style=\"color:yellow;background:#FF0000;\">★红底黄字</font>代表此值超过阀值。以下relay指本次getpath中用到的relay。";
	html += "<table class=\"detailTable\">" +
					"<tr align=\"center\">" +
	...
	html+="</table>";
	$("#relay0101_bz").html(html);
}

/**
 * Relay的状态信息，用在四个地方
 * Relay信息/短链关系/主叫端数据的顶部表格
 * Relay信息/RC输出/主叫端数据的顶部表格
 * Relay信息/短链关系/被叫端数据的顶部表格
 * Relay信息/RC输出/被叫端数据的顶部表格
 * @param {}
 * data
 */
function relayTable(){
	...
	if(mapRelay2Map!=undefined&&mapRelay2Map!=null&&mapRelay2Map!==""){
		mapRelay2Map.each(function(key,value,index){
			if(value!=undefined&&value!=null&&value!==""){
				if(mapRelay2MapisEnable!=undefined&&mapRelay2MapisEnable!=null&&mapRelay2MapisEnable!==""){
					if(mapRelay2MapisEnable.get(key)!=undefined&&mapRelay2MapisEnable.get(key)!=null&&mapRelay2MapisEnable.get(key)!==""){
			    		enable = mapRelay2MapisEnable.get(key);
					}
				}
	    		//ID
	...
	$("#relay0202_bz_01").html(html);//Relay信息/短链关系/被叫端数据的顶部表格
	$("#relay0402_bz_01").html(html);//Relay信息/RC输出/被叫端数据的顶部表格
}
														...
														$.each(link_info,function(j,va){
															html += "<div style='width:40px;float:left;border-bottom:thin solid #E69C39;'>" +
																	...
																	"<div style='width:70px;float:left;border-bottom:thin solid #E69C39;'>抖动："+
																		va.jitter+
																	"</div>"+
																	"<div style='width:320px;float:left;margin-right:20px;border-bottom:thin solid #E69C39;'>更新时间："+
																		va.updateTime+
																	"</div>" ;
														});
													}else{
...
/**
 * “Relay信息”标签页第二个子标签“短链关系”，被叫向数据
 * @param {}
 * data
 */
function relayView02_bz(datas){
	var result = datas.result;
	if(result === 0 ){
		var data = datas.data;
		if(data!=undefined&&data!=null&&data!==""){
...
/**
 * “Relay信息”标签页第三个子标签“可达路径”，被叫向数据
 */
function relayView03_bz(datas){
	var html = "";
	var html1 = "";
	var result = datas.result;
	if(result === 0){
				...
				if(getpath!=undefined&&getpath!=null&&getpath!==""){
					...
					if(reachable_path_array!=undefined&&reachable_path_array!=null&&reachable_path_array!==""){
						...
						html += "<table class='detailTable'>"+
									...
									"</tr>";
						for(var i=0;i<trNum;i++){
							html += "<tr align='center'><td colspan='12' align='center' id='relayView03_bz01"+i+"'></td></tr>";
						}
						html += "</table>";
					
						$.each(reachable_path_array,function(i,val){
							//用div来控制num个记录显示在同一行
							html1 += "<div id='relayView03_bz02"+i+"' style='width:356px;float:left;'>";
							html1 += "<table width='356px'>" ;
									html1 += "<tr>";
									segment = val.segment;
									quality = val.quality;
						...
						//把创建好的表格放到页面上，创建其文档元素
						$("#relay0301_bz").html(html);
						
						//把创建好的div放到页面上，创建其文档元素
						$("#relay0301_bz").append(html1);
						...
						for(var i = 0;i<trNum; i++){
							for(var j=0;j<num;j++){//每行存放num个记录
								++flag;
								if(flag <= length){
									$('#relayView03_bz01'+i).append($('#relayView03_bz02'+(flag-1)));
								}
							}
						}
					}else{
...
/**
 * “Relay信息”标签页第四个子标签“RC输出”，被叫向数据
 * @param {}
 * data
 */
function relayView04_bz(datas){
	var html ="";
	html += "<table class='detailTable'>" +
				"<tr>" +
				    "<th>path_id</th>" +
...
/**
 * “Relay信息”标签页第四个子标签“RC输出”，底部relay关系图，被叫向
 * @param {}
 * data
 */
function relayImg_bz(datas){
	var html = "";
	var result = datas.result;
	if(result ===0){
		var data = datas.data;
			...
			if(called!=undefined&&called!=null&&called!==""){
				...
				if(getpath!=undefined&&getpath!=null&&getpath!==""){
					...
					if(resp_cmd_path_array!=undefined&&resp_cmd_path_array!=null&&resp_cmd_path_array!==""){
						html += "<table class='detailTable'>";
						$.each(resp_cmd_path_array,function(i,val){
							html += "<tr><td id='relayImg_bz"+i+"'></td></tr>";
						});
						html += "</table>";
					
					    $('#relay0402_bz').append(html);
					
						$.each(resp_cmd_path_array,function(i,val){
							//画图函数
							drawPathByDataObject(val,"relayImg_bz"+i);
						});
					}else{
						html += "<table class='detailTable'>"+
									"<tr><td align='center'>无数据，无法展示Relay关系图!</td></tr>"+
							    "</table>";
					    $('#relay0402_bz').append(html);//必须写在此处，不能写在最后，否则表格行会重复
					}
				}else{
					html += "<table class='detailTable'>"+
								"<tr><td align='center'>无数据，无法展示Relay关系图!</td></tr>"+
