/**
 *falsh查询页面flashEventCallInfo.jsp的JS文件 
 */
 
 /*flashEventCallInfo.jsp页面中标签的JS效果 */
$(document).ready(function() {
			// 获取控制子标签内容的div
			var tabContainers = $('#macGuidEventCallInfoDiv > div');
			// 隐藏所有div，初始时显示第一个
			tabContainers.hide().filter(':first').show();

			$('.macGuidEventCall_tabs li a').click(function() {
						tabContainers.hide();// 隐藏所有
						tabContainers.filter(this.hash).show();// 显示当前标签的子内容
						// 将ul下所有标签的类名移除
						$('.macGuidEventCall_tabs li a').removeClass('selected');
						// 为当前点击的标签设置类名
 ...
 //全局变量，用于“在查询时间段内通过此mac、guid的所有通话记录 ”分页时作为参数使用，因为显示数据的方法showdata()在count.jsp页面也用，
 //在count.jsp中时间插件就在页面上，可以每次点击“首页/上一页/下一页/尾页”时都可以重新读取时间，不需要将时间作为参数传递到showdata()中。
 //而在flashEventCallInfo.jsp中，则无法读取插件时间，因为时间插件在flash_sdk_Search.jsp中，所以只能作为参数传递到此，赋值给全局变量，然后
 //showdata()每次递归调用时直接读取全局变量的值。
 var mac;
 var guid;
 var startTimeFlash ;//开始时间
 var endTimeFlash ;//结束时间
 
 /**
  * 将四个参数传输到后端，设置此页面所使用的四个全局变量，因为调用macGuidCallInfo(1)分页显示话务时，这四个变量要多次使用
  */
 function gainMacGuidStimeEtime(fmac,fguid,fstartTime,fendTime){
 	mac = fmac;
 	guid = fguid;
 	startTimeFlash = fstartTime;
 	endTimeFlash = fendTime;
 }
  ...
  */
function macGuidEventInfo_flashEventCallInfo(macs,guids,toPosition){
	$.ajax({
		type : "post",
		url : "search.macGuidEventInfo.action",
		dataType : "json",
		data : "mac=" + macs + "&guid=" + guids,
		beforeSend : function() {
		},
		complete : function() {
		},
		success : function(data) {
			...
			macGuidEventInfoView_flashEventCallInfo(data,toPosition,macs,guids);
 ...
 * @param {} data
 */
function macGuidEventInfoView_flashEventCallInfo(datas,toPosition,macs,guids){
	var result ;
	var html = "";
							...
							if(event_type=="event_login"&&sub_type=="f_login"){
								...
								html += "<div class='flashEventCallTitleDiv'>" +
										...
										"<table class='detailTable'>" +
											...
											"</tr>" +
											"<tr>" +
												"<td id='flashEventCallInfo_out_ip_"+i+"'>"+out_ip+"<br/></td>" +
												"<td>"+appkey+"</td>" +
												"<td>"+anony+"</td>" +
												"<td>"+real_login_num+"</td>" +
												"<td>"+login_time+"</td>" +
												"<td id='flashEventCallInfo_main_gateway_addr_"+i+"'>"+main_gateway_addr+"<br/></td>" +
												"<td id='flashEventCallInfo_back_gateway_addr_"+i+"'>"+back_gateway_addr+"<br/></td>" +
												"<td id='flashEventCallInfo_connect_gateway_addr_"+i+"'>"+connect_gateway_addr+"<br/></td>" +
										...
										if(out_ip!="无"&&out_ip!="null"){
											...
											if(out_ip_arr!=undefined&&out_ip_arr!=null&&out_ip_arr!==""&&out_ip_arr.length>0){
												...
												if(ip!=undefined&&ip!=null&&ip!==""){
													if(isIP(ip)){
														getIp(ip,'flashEventCallInfo_out_ip_'+i);
													}
												}
											}
										}
										...
										if(main_gateway_addr!="无"&&main_gateway_addr!="null"){
											...
											if(main_gateway_addr_arr!=undefined&&main_gateway_addr_arr!=null&&main_gateway_addr_arr!==""&&main_gateway_addr_arr.length>0){
												...
												if(ip!=undefined&&ip!=null&&ip!==""){
													if(isIP(ip)){
														getIp(ip,'flashEventCallInfo_main_gateway_addr_'+i);
													}
												}
											}
										}
										...
										if(back_gateway_addr!="无"&&back_gateway_addr!="null"){
											...
											if(back_gateway_addr_arr!=undefined&&back_gateway_addr_arr!=null&&back_gateway_addr_arr!==""&&back_gateway_addr_arr.length>0){
												...
												if(ip!=undefined&&ip!=null&&ip!==""){
													if(isIP(ip)){
														getIp(ip,'flashEventCallInfo_back_gateway_addr_'+i);
													}
												}
											}
										}
										...
										if(connect_gateway_addr!="无"&&connect_gateway_addr!="null"){
											...
											if(connect_gateway_addr_arr!=undefined&&connect_gateway_addr_arr!=null&&connect_gateway_addr_arr!==""&&connect_gateway_addr_arr.length>0){
												...
												if(ip!=undefined&&ip!=null&&ip!==""){
													if(isIP(ip)){
														getIp(ip,'flashEventCallInfo_connect_gateway_addr_'+i);
													}
												}
											}
										}
...
/**
 * 在查询时间段内通过此mac、guid的所有通话记录
 * @param {} currPage
 */
function macGuidCallInfo(currPage){
	
	$.ajax({
		type : "post",
		url : "search.macGuidCallInfo.action",
		dataType : "json",
		data : "currPage=" + currPage+ 
			   "&startTime=" + startTimeFlash + 
			   "&endTime=" + endTimeFlash +
			   "&mac=" + mac +
			   "&guid=" + guid +
			   "&reportType=4",//4：flash通话
		beforeSend : function() {
			$(".layer").show();
		},
		complete : function() {
		...
		success : function(data) {
			...
			var dd = "{\"result\":0,\"pageSize\":25,\"currPage\":1,\"count\":898,\"items\":[{\"sid\":\"4100586382_3819310494_76088422_71733177_1357815673_71733177\",\"caller\":0,\"called\":0,\"starttimes\":\"2015-05-18 11:18:55\",\"starttime\":1431919135000,\"endtimes\":\"2015-05-18-11:18:55\",\"durations\":\"0毫秒\",\"eventType\":-10000,\"punchresult\":\"\",\"pathtype\":0,\"pathtype2\":0,\"pathtype3\":0,\"mainpathlossr\":-1,\"backpathlossr\":-1,\"recvVideoLossr\":-1,\"recvAudioLossr\":-1,\"recvFecLossr\":-1,\"vformat\":\"\",\"limbps\":0,\"fecRatio\":\"\",\"maxBw\":0,\"curBw\":0,\"sendtoFailure\":0,\"reporttype\":2,\"reporttypecaller\":2,\"reporttypecalled\":2,\"pathSwitch\":0,\"appkey\":\"\",\"flash\":false},{\"sid\":\"4100586382_3819310494_78026420_73175519_1357815673_73175519\",\"caller\":0,\"called\":0,\"starttimes\":\"2015-05-18 11:18:55\",\"starttime\":1431919135000,\"endtimes\":\"2015-05-18-11:18:55\",\"durations\":\"0毫秒\",\"eventType\":-10000,\"punchresult\":\"\",\"pathtype\":0,\"pathtype2\":0,\"pathtype3\":0,\"mainpathlossr\":-1,\"backpathlossr\":-1,\"recvVideoLossr\":-1,\"recvAudioLossr\":-1,\"recvFecLossr\":-1,\"vformat\":\"\",\"limbps\":0,\"fecRatio\":\"\",\"maxBw\":0,\"curBw\":0,\"sendtoFailure\":0,\"reporttype\":2,\"reporttypecaller\":2,\"reporttypecalled\":2,\"pathSwitch\":0,\"appkey\":\"\",\"flash\":false},{\"sid\":\"4100586382_3819310494_79795153_72060466_1357815673_72060466\",\"caller\":79795153,\"called\":72060466,\"starttimes\":\"2015-05-18 11:18:55\",\"starttime\":1431919135000,\"endtimes\":\"2015-05-18 11:18:55\",\"durations\":\"0毫秒\",\"eventType\":-10000,\"punchresult\":\"relay\",\"pathtype\":0,\"pathtype2\":0,\"pathtype3\":3,\"mainpathlossr\":-1,\"backpathlossr\":-1,\"recvVideoLossr\":-1,\"recvAudioLossr\":-1,\"recvFecLossr\":-1,\"vformat\":\"VGA(640x480)\",\"limbps\":400,\"fecRatio\":\"4:2\",\"maxBw\":1024,\"curBw\":1024,\"sendtoFailure\":0,\"reporttype\":2,\"reporttypecaller\":2,\"reporttypecalled\":2,\"pathSwitch\":0,\"appkey\":\"\",\"flash\":false},{\"sid\":\"4100586382_3819310494_78444862_71337359_1357815673_78444862\",\"caller\":0,\"called\":0,\"starttimes\":\"2015-05-18 11:18:55\",\"starttime\":1431919135000,\"endtimes\":\"2015-05-18 11:18:55\",\"durations\":\"0毫秒\",\"eventType\":-10000,\"punchresult\":\"\",\"pathtype\":0,\"pathtype2\":0,\"pathtype3\":3,\"mainpathlossr\":-1,\"backpathlossr\":-1,\"recvVideoLossr\":-1,\"recvAudioLossr\":-1,\"recvFecLossr\":-1,\"vformat\":\"\",\"limbps\":0,\"fecRatio\":\"\",\"maxBw\":0,\"curBw\":0,\"sendtoFailure\":0,\"reporttype\":2,\"reporttypecaller\":2,\"reporttypecalled\":2,\"pathSwitch\":0,\"appkey\":\"appkvalue\",\"flash\":false}]}";
			var dds = JSON.parse(dd);
			//参数分别为：数据，函数名，显示到页面上的位置
			showdata(data, "macGuidCallInfo","macGuidCallInfo");
		},
		error : function(request, textStatus, errorThrown) {
			if (request.status == 900) {
				window.location.href = "login.jsp";
